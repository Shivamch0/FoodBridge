...
export const createDonation = asyncHandler(async (req, res) => {
  ...
  const donation = await FoodDonation.create({ ...req.body, images: uploadedImages.length ? uploadedImages : req.body.images, donor: req.user._id, status: "searching" });
  const organizations = await User.find({ role: "organization", isVerified: true }).select("_id");
  await createNotifications(organizations.map((organization) => organization._id), {
...
export const updateDonation = asyncHandler(async (req, res) => {
  ...
  const editableFields = ["foodName", "foodType", "description", "quantity", "unit", "preparedAt", "expiresAt", "images", "pickupLocation", "deliveryPreference"];
  for (const field of editableFields) {
    if (req.body[field] !== undefined) donation[field] = req.body[field];
  }
  if (new Date(donation.expiresAt) <= new Date()) throw new ApiError(400, "expiresAt must be in the future");
  await donation.save();
